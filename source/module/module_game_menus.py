...
##++START-VEXED++##
           (rest_for_hours_interactive, 24 * 365, 5, 0), #rest while attackable
##++END-VEXED++##
           (change_screen_return),
        ]
       ),
...
##++END-VEXED++##
      ...
      ##diplomacy end+
      ("action_modify_banner",[(this_or_next|gt, "$cheat_mode", 0),(ge, "$g_player_banner_granted", 1)],"Modify your banner (Shift to change colour).",
       [
        #SB : recolor mode
        (try_begin),
          (this_or_next|key_is_down, key_left_shift),
...
##++START-VEXED++##
      ...
      (store_random_in_range, ":rand", 36, 100),
##++END-VEXED++##
...
##++START-VEXED++##
          (rest_for_hours_interactive, ":time_to_wait", 5, 0), #rest while attackable
##++END-VEXED++##
...
##++START-VEXED++##
          (val_mul, "$qst_eliminate_bandits_infesting_village_num_bandits", 1250),
          (val_div, "$qst_eliminate_bandits_infesting_village_num_bandits", 1000),
##++END-VEXED++##
...
##++END-VEXED++##
      ...
      ("collect_taxes_qst",[(party_slot_eq, "$current_town", slot_village_state, svs_normal),
                            ...
                            (party_slot_eq, "$curren_town", slot_town_lord, ":quest_giver_troop"),
...
##++START-VEXED++##
     (store_sub, ":multiplier", 11, ":max_skill"),
     (val_mul, ":improvement_cost", ":multiplier"),
     (val_div, ":improvement_cost", 20),

     (store_div, ":improvement_time", ":improvement_cost", 400),
##++END-VEXED++##
    ...
    [
      ("peasants_against_bandits_attack_resist", [], "Prepare for a fight!",
       [ ## SB : use new bandit script
        ...
        (val_max, ":num_villagers", ":bandit_troop"),
    ...
    [
      ...
      ("trade_with_horse_merchant",[(party_slot_ge, "$current_town", slot_town_horse_merchant, 1)],
       ...
       [
            ...
            (else_try),
              (play_sound, "snd_neigh"), #horse_low_whinny
              (troop_set_slot, ":merchant_troop", slot_troop_met, 1),
              (change_screen_trade, ":merchant_troop"),
            (try_end),
        ]),
...
#          (try_end),
  ...
  ("start_phase_2_5",mnf_disable_all_keys,
    ...
    [
    #SB : skip quest
      ("skip_quest",[], "[Skip Tutorial] Take a nice long walk around {s1}...",
       [
...
##++END-VEXED++##
      ...
      ("continue",[], "[Tutorial] Go find an inn...",
       [
 ...
 AI Behaviour is {s13}{reg18? (currently {s14}):}, Object is {s15}{reg19? (currently {s16}):} at ({reg20},{reg21})",
  ...
  ( #helper menu to show all troop slots
    ...
    #So apparently this one needs to re-jump to the menu
      ...
      ("next",
      [
        (lt, "$g_talk_troop", multiplayer_coop_companion_equipment_sets_end), #last troop apparently
